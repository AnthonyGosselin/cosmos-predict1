...
def parse_args() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--dataset_path", type=str, default="datasets/mmau_videos_ar", help="Root path to the dataset")
...
def init_t5(
    ...
    print(f"Loading text encoder {pretrained_model_name_or_path} from {cache_dir}...")
...
def main(args) -> None:
    ...
    with open(os.path.join(args.dataset_path, "paired_data_val.jsonl"), "r") as f:
        val_paired_data = [json.loads(line) for line in f]

    with open(os.path.join(args.dataset_path, "paired_data_train.jsonl"), "r") as f:
        train_paired_data = [json.loads(line) for line in f]

    all_paired_data = val_paired_data + train_paired_data
    ...
    for sample_dict in tqdm(all_paired_data):
        prompt = sample_dict["prompt"]
        visual_input = sample_dict["visual_input"]
        sample_name = visual_input.split("/")[-1].split(".")[0]

        t5_xxl_filename = os.path.join(t5_xxl_dir, f"{sample_name}.pickle")
    ...
    print("Done generating T5 text embeddings.")
